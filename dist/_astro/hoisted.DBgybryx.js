import{r as D}from"./hoisted.DHugXsqa.js";function F(t){const a=t.weight/Math.pow(t.height/100,2),r=Math.round(a*10)/10;let e="",o="",s="";return a<18.5?(e="Underweight",o="#0070f3",s="Consider consulting a nutritionist to reach a healthy weight"):a<25?(e="Normal",o="#50e3c2",s="Great! Maintain your current lifestyle"):a<30?(e="Overweight",o="#f5a623",s="Consider a balanced diet and regular exercise"):(e="Obese",o="#ff6b6b",s="Consult a healthcare professional for a personalized plan"),{value:r.toString(),unit:"BMI",category:e,color:o,advice:s}}function S(t){const{weight:a,height:r,age:e,gender:o}=t;let s=10*a+6.25*r-5*e;return s+=o==="male"?5:-161,{value:Math.round(s).toString(),unit:"kcal/day",advice:"This is your baseline calorie burn at complete rest"}}function C(t){const a=S(t),r=parseInt(a.value),e={sedentary:1.2,light:1.375,moderate:1.55,very:1.725,athlete:1.9};return{value:Math.round(r*e[t.activity]).toString(),unit:"kcal/day",advice:"Your total daily calorie burn including activity",breakdown:{bmr:a.value,multiplier:e[t.activity]}}}function L(t){const{gender:a,height:r,waist:e,neck:o,hip:s}=t;let i;if(a==="male"){const d=Math.log10(e-o),b=Math.log10(r);i=495/(1.0324-.19077*d+.15456*b)-450}else{const d=Math.log10(e+s-o),b=Math.log10(r);i=495/(1.29579-.35004*d+.221*b)-450}const c=Math.round(i*10)/10;let l="",n="";return a==="male"?i<6?(l="Essential",n="#0070f3"):i<14?(l="Athletes",n="#50e3c2"):i<18?(l="Fitness",n="#50e3c2"):i<25?(l="Average",n="#f5a623"):(l="Obese",n="#ff6b6b"):i<14?(l="Essential",n="#0070f3"):i<21?(l="Athletes",n="#50e3c2"):i<25?(l="Fitness",n="#50e3c2"):i<32?(l="Average",n="#f5a623"):(l="Obese",n="#ff6b6b"),{value:c.toString(),unit:"%",category:l,color:n}}function P(t){const{gender:a,height:r}=t,o=r/2.54-60,s=a==="male"?52+1.9*o:49+1.7*o,i=a==="male"?56.2+1.41*o:53.1+1.36*o,c=a==="male"?48+2.7*o:45.5+2.2*o,l=a==="male"?50+2.3*o:45.5+2.3*o,n=22*Math.pow(r/100,2),d=[s,i,c,l,n],b=Math.round(Math.min(...d)),p=Math.round(Math.max(...d));return{value:`${b}–${p}`,unit:"kg",advice:"Range based on 5 medical formulas"}}function T(t){const a=C(t),r=parseInt(a.value),e={lose:-500,maintain:0,build:300},o=r+e[t.goal],s={lose:"0.5kg loss/week",maintain:"maintain weight",build:"0.25kg gain/week"};return{value:o.toString(),unit:"kcal/day",advice:s[t.goal],breakdown:{tdee:r.toString(),adjustment:e[t.goal]}}}function A(t){const a=T(t),r=parseInt(a.value),e=Math.round(t.weight*2),o=e*4,s=Math.round(r*.25),i=Math.round(s/9),c=r-o-s,l=Math.round(c/4);return{value:`${e}g / ${i}g / ${l}g`,unit:"P / F / C",breakdown:{protein:e,fat:i,carbs:l,calories:r}}}function j(t){const a=t.weight*35,r=(t.exerciseMin||0)/30*350,e=Math.round((a+r)/1e3*10)/10;return{value:e.toString(),unit:"L/day",advice:`${Math.round(e*4)} cups per day`,breakdown:{base:Math.round(a),exercise:Math.round(r)}}}function z(t){const a=220-t.age,r=t.restHR||70,e=a-r,o=`${Math.round(r+e*.5)}–${Math.round(r+e*.6)}`,s=`${Math.round(r+e*.6)}–${Math.round(r+e*.7)}`,i=`${Math.round(r+e*.7)}–${Math.round(r+e*.85)}`;return{value:i,unit:"bpm",advice:"Target for cardio training",breakdown:{warmup:o,fatburn:s,cardio:i,max:a}}}function H(t){const{gender:a,weight:r,height:e}=t;let o;a==="male"?o=.407*r+.267*e-19.2:o=.252*r+.473*e-48.3;const s=r-o,i=s/r*100;return{value:Math.round(o).toString(),unit:"kg",advice:`Fat mass: ${Math.round(s)}kg (${Math.round(i)}%)`,breakdown:{lbm:Math.round(o),fat:Math.round(s)}}}function N(t){const a=t.weight||60,r=t.reps||5,e=t.lift||"Lift",o=a*(1+r/30),s=Math.round(o*10)/10;let i="";return r<=3?i="Highly accurate":r<=6?i="Good accuracy":i="Moderate accuracy",{value:s.toString(),unit:"kg",category:e,breakdown:{"90%":Math.round(s*.9),"80%":Math.round(s*.8),"70%":Math.round(s*.7),"60%":Math.round(s*.6)},advice:i,color:r<=3?"#50e3c2":r<=6?"#f5a623":"#0070f3"}}function y(t){const a=Math.floor(t/3600),r=Math.floor(t%3600/60),e=Math.round(t%60);return a>0?`${a}:${r.toString().padStart(2,"0")}:${e.toString().padStart(2,"0")}`:`${r}:${e.toString().padStart(2,"0")}`}function O(t){const a=t.distance||10;let r=0,e=0,o=0;t.duration&&typeof t.duration=="object"?(r=t.duration.hours??0,e=t.duration.minutes??0,o=t.duration.seconds??0):(r=t.hours??0,e=t.minutes??45,o=t.seconds??0);const s=r*3600+e*60+o,i=s/a;let c=Math.floor(i/60),l=Math.round(i%60);l===60&&(l=0,c+=1);const n=`${c}:${l.toString().padStart(2,"0")}/km`;let d="Intermediate";i<240?d="Elite":i<300?d="Competitive":i<360?d="Strong":i<420?d="Intermediate":d="Beginner";const b=[],p=Math.min(Math.ceil(a),50);for(let g=1;g<=p;g++){const u=Math.min(g*i,s),h=g===Math.ceil(a)&&a%1!==0?a%1*i:i;b.push({km:g,splitTime:y(h),cumulativeTime:y(u)})}const m=[{name:"5K",dist:5},{name:"10K",dist:10},{name:"Half Marathon",dist:21.0975},{name:"Marathon",dist:42.195}].map(g=>{const u=s*Math.pow(g.dist/a,1.06),h=u/g.dist;let $=Math.floor(h/60),w=Math.round(h%60);return w===60&&(w=0,$+=1),{name:g.name,time:y(u),pace:`${$}:${w.toString().padStart(2,"0")}/km`}});return{value:n,unit:"pace",category:d,breakdown:{pace:n,distance:a.toFixed(2),duration:y(s),splits:b,predictions:m}}}function W(t){const a=t.weight||70,r=t.height||170,e=t.age||30,o=t.gender||"male",s=t.activity||"moderate",i=t.deficitAmount||500;let c=10*a+6.25*r-5*e;c+=o==="male"?5:-161;const n=Math.round(c*{sedentary:1.2,light:1.375,moderate:1.55,very:1.725,athlete:1.9}[s]),d=o==="male"?1500:1200,b=Math.max(n-i,d),p=i*7/7700,m=p*4.33;return{value:b.toString(),unit:"kcal/day",breakdown:{tdee:n,deficit:i,weeklyLoss:p.toFixed(2),monthlyLoss:m.toFixed(2),minSafe:d}}}function Y(t){const a=t.sleepMode==="bedtime"?t.wakeTime:t.sleepTime,r=t.sleepMode||"bedtime";if(!a)return{value:"06:30",unit:"time",advice:"Enter a time to calculate"};const e=[2,3,4,5,6],o=14,s=90;let i=0,c=0;if(typeof a=="object"&&a!==null)i=a.hours??7,c=a.minutes??0;else if(typeof a=="string"&&a.includes(":")){const b=a.split(":").map(Number);i=b[0]??7,c=b[1]??0}else i=7,c=0;const l=i*60+c,n={};let d="";return e.forEach(b=>{const p=b*s+o;let m=r==="bedtime"?l-p:l+p;for(;m<0;)m+=1440;for(;m>=1440;)m-=1440;const g=Math.floor(m/60),u=Math.round(m%60),h=`${String(g).padStart(2,"0")}:${String(u).padStart(2,"0")}`;n[`${b}cycles`]=h,b===5&&(d=h)}),{value:d,unit:"time",breakdown:n,advice:"5 cycles = 7.5 hours (optimal)"}}function G(t){const a=t.weight||70,r=t.bodyFat||20,e=t.proteinGoal||"maintain",o=a*(1-r/100),s={maintain:{min:1.2,max:1.6},loss:{min:1.8,max:2.4},gain:{min:1.6,max:2.2},recomp:{min:2,max:2.6},athlete:{min:1.7,max:2.5}},i=s[e]||s.maintain,c=Math.round(o*i.min),l=Math.round(o*i.max),n=Math.round((c+l)/2);return{value:`${c}–${l}`,unit:"g/day",breakdown:{floor:c,target:n,ceiling:l,perKg:i.min.toFixed(1),calories:n*4}}}function U(t){const{gender:a,height:r,neck:e,waist:o,hip:s,age:i}=t;let c;a==="male"?c=86.01*Math.log10(o-e)-70.041*Math.log10(r)+36.76:c=163.205*Math.log10(o+s-e)-97.684*Math.log10(r)-78.387;const l={male:{"17-20":20,"21-27":22,"28-39":24,"40+":26},female:{"17-20":30,"21-27":32,"28-39":34,"40+":36}};let n="40+";i<=20?n="17-20":i<=27?n="21-27":i<=39&&(n="28-39");const d=l[a][n],b=c<=d;return{value:(Math.round(c*10)/10).toString(),unit:"%",category:b?"✓ PASS":"✗ FAIL",color:b?"#50e3c2":"#ff6b6b",breakdown:{standard:d,ageGroup:n,difference:Math.round((c-d)*10)/10}}}const M={bmi:{label:"BMI Calculator",description:"Body Mass Index",metaDescription:"Free BMI calculator — enter your height and weight to instantly find your Body Mass Index, understand your result, and see what it means for your health.",icon:"⚖️",fields:["age","gender","height","weight"],fn:F},bmr:{label:"BMR Calculator",description:"Basal Metabolic Rate",metaDescription:"Calculate your Basal Metabolic Rate (BMR) free — find the exact calories your body burns at rest using the science-backed Mifflin-St Jeor equation.",icon:"🔥",fields:["age","gender","height","weight"],fn:S},tdee:{label:"TDEE Calculator",description:"Total Daily Energy Expenditure",metaDescription:"Free TDEE calculator — find your Total Daily Energy Expenditure based on your activity level. Know exactly how many calories you burn each day.",icon:"⚡",fields:["age","gender","height","weight","activity"],fn:C},bodyfat:{label:"Body Fat %",description:"US Navy Method",metaDescription:"Estimate your body fat percentage free — uses the Navy method and BMI formula. Understand your result with healthy range charts and next steps.",icon:"📊",fields:["gender","height","waist","neck","hip"],fn:L},idealweight:{label:"Ideal Weight",description:"Based on 5 formulas",metaDescription:"Find your ideal weight free — calculated using 5 medical formulas (Devine, Robinson, Miller, Hamwi, BMI method) so you get a realistic healthy range.",icon:"🎯",fields:["gender","height"],fn:P},calories:{label:"Calorie Target",description:"Daily calorie goal",metaDescription:"Free daily calorie intake calculator — set your weight goal and get your exact calorie target for weight loss, maintenance, or muscle gain.",icon:"🍎",fields:["age","gender","height","weight","activity","goal"],fn:T},macros:{label:"Macro Split",description:"Protein, carbs, fat breakdown",metaDescription:"Free macro calculator — get your personalised protein, carbohydrate, and fat targets based on your TDEE and fitness goal.",icon:"🥗",fields:["age","gender","height","weight","activity","goal"],fn:A},water:{label:"Water Intake",description:"Daily hydration goal",metaDescription:"Calculate your daily water intake free — get a hydration goal based on your weight, activity level, and climate.",icon:"💧",fields:["weight","exerciseMin"],fn:j},heartrate:{label:"Target Heart Rate",description:"Training zones",metaDescription:"Free target heart rate calculator — find your fat-burn, cardio, and peak training zones based on your age and resting heart rate.",icon:"❤️",fields:["age","restHR"],fn:z},leanmass:{label:"Lean Body Mass",description:"Muscle vs fat mass",metaDescription:"Calculate your lean body mass free — estimate your muscle mass vs fat mass using the Boer and James formulas.",icon:"💪",fields:["gender","height","weight"],fn:H},onerep:{label:"1-Rep Max Calculator",description:"Find your maximum strength",metaDescription:"Free 1-rep max calculator — enter the weight you lifted and reps completed to instantly estimate your 1RM and get training weights for strength, hypertrophy, and endurance.",icon:"🏋️",fields:["lift","weight","reps"],fn:N},pace:{label:"Running Pace Calculator",description:"Calculate pace and finish time",metaDescription:"Free running pace calculator — find your pace per km, predicted finish time, or total distance. Supports 5K, 10K, half marathon, and full marathon with per-km split table.",icon:"🏃",fields:["distance","duration"],fn:O},deficit:{label:"Calorie Deficit Calculator",description:"Find your weight loss target",metaDescription:"Free calorie deficit calculator — enter your stats and goal to find your exact daily calorie target, weekly fat loss projection, and safe minimum intake. Based on Mifflin-St Jeor.",icon:"🎯",fields:["gender","age","height","weight","activity","deficitAmount"],fn:W},sleep:{label:"Sleep Calculator",description:"Find ideal sleep times",metaDescription:"Free sleep calculator — enter your wake-up time or bedtime to find the optimal sleep and wake times aligned with 90-minute sleep cycles so you wake up refreshed, not groggy.",icon:"😴",fields:["sleepMode","wakeTime","sleepTime"],fn:Y},protein:{label:"Protein Intake Calculator",description:"Your daily protein target",metaDescription:"Free protein intake calculator — get your personalised daily protein target in grams based on your body weight, goal (muscle gain, weight loss, recomp), and training frequency.",icon:"🥩",fields:["weight","bodyFat","proteinGoal"],fn:G},armybf:{label:"Army Body Fat Calculator",description:"Official military standard",metaDescription:"Free Army body fat calculator — uses the official US Army circumference formula to estimate body fat percentage and check your result against Army standards for your age and sex.",icon:"🪖",fields:["gender","age","height","neck","waist","hip"],fn:U}},x=window.location.pathname.split("/")[2],B=M[x],R=JSON.parse(localStorage.getItem(`fc_calc_${x}`)||"{}"),f=B.fn(R),v=D();v.milestone&&v.message&&setTimeout(()=>{window.confetti&&confetti({particleCount:100,spread:70,origin:{y:.6}}),window.showNotification(v.message,"milestone")},500);J(f,x,R);window.addEventListener("themeChanged",()=>{I(f,x)});K();function K(){const t=JSON.parse(localStorage.getItem("fc_history")||"[]"),a={calculator:B.label,slug:x,value:f.value,unit:f.unit,date:new Date().toLocaleDateString(),timestamp:Date.now()};if(!(t.length>0&&t[0].slug===x&&t[0].value===f.value&&t[0].date===a.date)){t.unshift(a),localStorage.setItem("fc_history",JSON.stringify(t.slice(0,50)));const e=t.slice(0,3).map(o=>({calculator:o.calculator,value:`${o.value} ${o.unit}`,date:o.date}));localStorage.setItem("fc_last_results",JSON.stringify(e))}}document.getElementById("save-result").onclick=()=>{window.showNotification("Result is automatically saved to your history")};function J(t,a,r){const e=document.getElementById("dashboard-root");e.innerHTML=`
    ...
    `).join("")}function X(t){const a=E(t),r=document.getElementById("related-calcs");r.innerHTML=a.map(e=>{const o=M[e];return`
